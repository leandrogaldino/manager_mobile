...
class SyncService {
  ...
  Future<int> runSync({bool isAuto = false}) async {
    ...
    try {
      ...
      log('Sincronizando PersonCompressorCoalescent...', time: DateTime.now());
      final int personCompressorCoalescentCount = await _personCompressorcoalescentService.synchronize(lastSync);
      log('$personCompressorCoalescentCount PersonCompressorCoalescent sincronizados... (Total: $totalCount)', time: DateTime.now());
      totalCount += personCompressorCoalescentCount;
      log('Sincronizando PersonCompressors...', time: DateTime.now());
      final int personCompressorCount = await _personCompressorService.synchronize(lastSync);
      log('$personCompressorCount PersonCompressors sincronizados... (Total: $totalCount)', time: DateTime.now());
