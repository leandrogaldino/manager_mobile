...
class CompressorInterfaceRepository {
  ...
  Future<Map<String, Object?>> getById(int id) async {
    try {
      ...
      const code = 'CIN001';
      const message = 'Erro ao obter interface';
    ...
    try {
      ...
      while (hasMore) {
        ...
        final remoteResult = await _remoteDatabase.get(
          collection: 'compressorinterfaces',
          filters: [
        ...
        for (final data in remoteResult) {
          ...
          final exists = await _localDatabase.isSaved('compressorinterface', id: id);
          ...
          if (exists) {
            ...
            await _localDatabase.insert('compressorinterface', data);
          }
          count++;
      ...
      const code = 'CIN002';
      const message = 'Erro ao sincronizar interfaces';
      log('[$code] $message', time: DateTimeHelper.now(), error: e, stackTrace: s);
      throw RepositoryException(code, message);
    }
  }
